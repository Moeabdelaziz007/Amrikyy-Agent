...
class OpenRouterClient {
  ...
  calculateCost(usage, modelInfo) {
    if (!usage || !modelInfo.cost) return 0;
  ...
  async generateTravelRecommendations(preferences) {
    const messages = [
      {
        role: 'system',
        content: 'You are Maya, a professional travel agent. Provide detailed, personalized travel recommendations based on user preferences.'
      },
      {
        role: 'user',
