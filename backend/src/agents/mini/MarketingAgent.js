...
class MarketingAgent {
  constructor() {
    ...
    this.genAI = new GoogleGenerativeAI({ apiKey: process.env.API_KEY }); // Using process.env.API_KEY as per guidelines
    this.model = this.genAI.getGenerativeModel({
      model: process.env.GEMINI_MODEL || 'gemini-2.5-flash' // Using a general-purpose model for now
    });

    if (!process.env.API_KEY) { // Changed to API_KEY
      logger.warn('[MarketingAgent] API_KEY is not set. Marketing Agent will not be able to make real AI calls.');
    }
