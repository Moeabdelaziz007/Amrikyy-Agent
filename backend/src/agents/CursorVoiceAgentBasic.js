/**
 * Basic Cursor Voice Agent - No Advanced Dependencies
 * Simple voice integration for testing without Bottleneck, Prometheus, etc.
 */
...
class CursorVoiceAgentBasic extends EventEmitter {
  constructor(cursorManager) {
    super();

    this.agent_id = "cursor-voice-basic";
    ...
    // Simple console logging instead of winston
    this.logger = {
    ...
    this.logger.info('🎤 Basic Cursor Voice Agent initialized');
  }
  ...
  async storeSessionData(session) {
    try {
      ...
      // Simple file storage (no advanced logging)
  ...
  simulateTranscription(voiceData) {
    ...
    const sampleTranscriptions = [
      ...
      "Hello Cursor, this is a test voice input"
    ];
...
module.exports = CursorVoiceAgentBasic;
