...
/**
 * @const {string} AGENT_TOOLBELT
 * @description A string constant that defines the capabilities of all available Mini Agents.
 * This is used to provide context to the OrchestratorAgent when it plans a workflow.
 * @private
 */
...
/**
 * @const {object} WORKFLOW_SCHEMA
 * @description The JSON schema for the workflow plan generated by the OrchestratorAgent.
 * This schema is used to enforce the structure of the AI's output.
 * @private
 */
 ...
 * @description An agent that understands user intent, generates dynamic workflows, and assigns tasks to Mini Agents using Gemini Pro.
 */
class OrchestratorAgent {
  /**
   * @constructor
   * @description Initializes the OrchestratorAgent and its sub-agents.
   */
  constructor() {
   ...
   * This method first refines the user's prompt using the PromptEngineeringAgent, then uses the refined prompt
   * to generate a structured workflow plan by calling the Gemini API.
   * @param {string} prompt - The natural language prompt from the user.
   * @returns {Promise<object>} A structured workflow plan that conforms to WORKFLOW_SCHEMA.
   */
