...
class PrivateJournalServer {
    constructor(journalPath) {
        this.journalManager = new journal_1.JournalManager(journalPath);
        this.searchService = new search_1.SearchService(journalPath);
                        ...
                        content: [
                            {
                                ...
                                text: results.length > 0
                                    ? `Found ${results.length} relevant entries:\n\n${results.map((result, i) => `${i + 1}. [Score: ${result.score.toFixed(3)}] ${new Date(result.timestamp).toLocaleDateString()} (${result.type})\n` +
                        ...
                        content: [
                            {
                                type: 'text',
                                text: results.length > 0
                                    ? `Recent entries (last ${days} days):\n\n${results.map((result, i) => `${i + 1}. ${new Date(result.timestamp).toLocaleDateString()} (${result.type})\n` +
                                        `   Sections: ${result.sections.join(', ')}\n` +
                                        `   Path: ${result.path}\n` +
                                        `   Excerpt: ${result.excerpt}\n`).join('\n')}`
