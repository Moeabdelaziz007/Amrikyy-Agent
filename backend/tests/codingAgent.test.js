...
describe('CodingAgent', () => {
  ...
  process.env.GEMINI_API_KEY = mockApiKey; // Set mock API key
  ...
  test('should throw error if GEMINI_API_KEY is not set', async () => {
    ...
    await expect(agent.executeTask({ type: 'generateUI', projectDescription: 'test' }))
      .rejects.toThrow('GEMINI_API_KEY is not configured. Cannot make real AI calls.');
  });
  ...
  test('reviewCode should call Gemini with correct prompt and return response', async () => {
    ...
    expect(callArgs.contents[0].parts[0].text).toContain(`Code to Review:\n\`\`\`\n${codeToReview}\n\`\`\``);
    expect(result).toEqual({
      success: true,
